    ...
    internal partial class Modifier
    {
        ...
        )
        {
            foreach(var changer in changers)
            {
                ...
                foreach(var toggler in changer.costumes[changer.defaultCostume].parametersPerMenu.objects)
                {
                ...
                if(root) AnimationHelper.AddCostumeChangerTree(controller, clips, name, changer.defaultCostume, root);
                else AnimationHelper.AddCostumeChangerLayer(controller, hasWriteDefaultsState, clips, name, changer.defaultCostume);

                #if LIL_VRCSDK3A
                // パラメーターを追加
                parameters.AddParameterInt(name, changer.isLocalOnly, changer.isSave, changer.defaultCostume);
                #endif
            }
        }
    }
