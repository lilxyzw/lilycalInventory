...
namespace jp.lilxyzw.lilycalinventory
{
    internal class InternalMenu
    {
        ...
        {
            return new InternalMenu(){type = InternalMenuType.Folder, name = name, icon = icon};
        }

        internal static InternalMenu CreateToggle(string name, Texture2D icon, string parameterName, float value = 1)
        {
            return new InternalMenu(){type = InternalMenuType.Toggle, name = name, icon = icon, parameterName = parameterName, value = value};
        }

        internal static InternalMenu CreateSlider(string name, Texture2D icon, string parameterName, float value = 1)
        {
            return new InternalMenu(){type = InternalMenuType.Slider, name = name, icon = icon, parameterName = parameterName, value = value};
        }

        internal static InternalMenu CreateTrigger(string name, Texture2D icon, string parameterName, float value = 1)
        {
            return new InternalMenu(){type = InternalMenuType.Trigger, name = name, icon = icon, parameterName = parameterName, value = value};
        }
    }
